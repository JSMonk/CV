...
\section[title={Artem Kobzar 1111},reference={artem-kobzar-1111}]
...
\item
  ...
  Primero]\from[url10],
  \useURL[url11][https://crossoverhealth.com/][][Crossover
  Health]\from[url11],
  \useURL[url12][https://doc.ai/][][doc.ai]\from[url12],
  \useURL[url13][https://www.nextlead.io/][][NextLead]\from[url13].
\stopitemize
...
\useURL[url14][https://nodejs.org/][][Node.js]\from[url14] and popular
backend frameworks like
(\useURL[url15][https://nestjs.com/][][NestJS]\from[url15],
\useURL[url16][https://expressjs.com/ru/][][Express]\from[url16],
\useURL[url17][https://koajs.com/][][Koa]\from[url17],
\useURL[url18][https://sailsjs.com/][][Sails]\from[url18]) at the
backend side.

At the backend, I used different DB solutions:
\useURL[url19][https://www.mysql.com/][][MySQL]\from[url19],
\useURL[url20][https://www.sqlite.org/][][SQLite]\from[url20],
\useURL[url21][https://www.postgresql.org/][][PostgreSQL]\from[url21],
\useURL[url22][https://www.mongodb.com/][][MongoDB]\from[url22],
\useURL[url23][https://redis.io/][][Redis]\from[url23],
\useURL[url24][https://aws.amazon.com/dynamodb][][DynamoDB]\from[url24].

Additionally, I worked a lot with popular front-end frameworks like
\useURL[url25][https://reactjs.org/][][React]\from[url25]
(\useURL[url26][https://reactnative.dev/][][React Native]\from[url26]
too) and \useURL[url27][https://vuejs.org/][][Vue.js]\from[url27].

And of course, I worked with popular static type solutions for
JavaScript:
\useURL[url28][https://www.typescriptlang.org/][][TypeScript]\from[url28]
and \useURL[url29][https://flow.org/][][Flow.js]\from[url29].

{\bf \quotation{Secondary} Technologies}

I worked with \type{C/C++} and
\useURL[url30][https://www.rust-lang.org/][][Rust]\from[url30]
ecosystem, especially when I was working with cryptography and
cryptocurrencies.

Also, I worked with \useURL[url31][https://dart.dev/][][Dart
Language]\from[url31] and his ecosystem, when I was working in
\useURL[url32][https://www.wrike.com/][][Wrike Company]\from[url32].
...
\useURL[url33][https://www.php.net/][][PHP]\from[url33] and
\useURL[url34][https://www.ruby-lang.org/ru/][][Ruby]\from[url34]
(\useURL[url35][https://rubyonrails.org/][][RoR]\from[url35] and
\useURL[url36][http://sinatrarb.com/][][Sinatra]\from[url36]).

{\bf \quotation{Basic Knowledge} Technologies}

In my spare time, I really like to work with:
\useURL[url37][https://www.haskell.org/][][Haskell]\from[url37],
\useURL[url38][https://www.idris-lang.org/][][Idris]\from[url38],
\useURL[url39][https://reasonml.github.io/][][ReasonML]\from[url39]/\useURL[url40][https://ocaml.org/][][OCaml]\from[url40],
\useURL[url41][https://elixir-lang.org/][][Elixir]\from[url41],
\useURL[url42][https://clojure.org/][][Clojure]\from[url42] and
\useURL[url43][https://golang.org/][][Golang]\from[url43]. Additionally,
I basically know
\useURL[url44][https://www.java.com][][Java]\from[url44],
\useURL[url45][https://www.apple.com/swift/][][Swift]\from[url45],
\useURL[url46][https://www.python.org/][][Python]\from[url46].
...
\startitemize
\item
  \useURL[url47][https://github.com/JSMonk/hegel][][Hegel]\from[url47] -
  a static type checker for JavaScript with high-level type inference
  (Hindley-Milner algorithm) and a strong type system.
\item
  \useURL[url48][https://github.com/JSMonk/sweet-monads][][sweet-monads]\from[url48]
...
\startitemize
\item
  \useURL[url49][https://github.com/paulmillr/chokidar][][chokidar]\from[url49]
  - the most popular Node.js file watcher. The 3.0.0 version was
  developed on my own.
\item
  \useURL[url50][https://github.com/paulmillr/readdirp][][readdirp]\from[url50]
  - recursive \type{readdir} with additional goodies (15,603,219
  downloads per week). The 3.0.0 version was developed on my own.
\item
  \useURL[url51][https://github.com/paulmillr/noble-ripemd160][][noble-ripemd160]\from[url51]
  - a cryptography hash function.
\item
  \useURL[url52][https://github.com/paulmillr/noble-ed25519][][noble-ed25519]\from[url52]
  - a digital signature library based at ed15519 schema.
\item
  \useURL[url53][https://github.com/paulmillr/noble-secp256k1][][noble-secp256k1]\from[url53]
  - a digital signature library based at secp256k1 schema.
\item
  \useURL[url54][https://github.com/paulmillr/noble-bls12-381][][noble-bls12-381]\from[url54]
...
over to \useURL[url55][https://github.com/paulmillr][][Paul
Miller]\from[url55]:
\stopblockquote
...
\item
  I teach students at
  \useURL[url56][http://javascript.ninja/][][JavaScript.Ninja
  Project]\from[url56]
\item
  I and my friend make a Russian-language podcast
  \useURL[url57][https://underjs.ru/][][UnderJS]\from[url57] about
  JavaScript and Compilers.
\item
  I also one of the organizers one of the largest conference in
  Russian-language countries
  \useURL[url58][https://holyjs.ru/][][HolyJS]\from[url58]
\item
  And also, I'm a public speaker. For now, I speak only for the Russian
  language audience. My recent talks:
  \useURL[url59][https://www.youtube.com/watch?v=JKvmwOuqVWI][][\quotation{Dissection
  of Dart VM}]\from[url59],
  \useURL[url60][https://www.youtube.com/watch?v=S0cCjbWuvzk&t=39s][][\quotation{Either
  monad: theory and practice}]\from[url60],
  \useURL[url61][https://www.youtube.com/watch?v=GIHrPm_YAIc&t=1715s][][\quotation{Why
  and How I write one more static type checker}]\from[url61].
\item
  English level: B1/B2
\stopitemize
